...
ICACHE_FLASH_ATTR void esp_spiffs_init() {
#else
...
ICACHE_FLASH_ATTR void esp_spiffs_deinit()
{
    free(work_buf.buf);
    work_buf.buf = 0;
...
ICACHE_FLASH_ATTR int32_t esp_spiffs_mount()
{
...
ICACHE_FLASH_ATTR long _write_filesystem_r(struct _reent *r, int fd, const char *ptr, int len )
{
    return SPIFFS_write(&fs, (spiffs_file)fd, (char*)ptr, len);
}

// This implementation replaces implementation in core/newlib_syscals.c
ICACHE_FLASH_ATTR long _read_filesystem_r( struct _reent *r, int fd, char *ptr, int len )
{
    return SPIFFS_read(&fs, (spiffs_file)fd, ptr, len);
}

ICACHE_FLASH_ATTR int _open_r(struct _reent *r, const char *pathname, int flags, int mode)
{
...
// This implementation replaces implementation in core/newlib_syscals.c
ICACHE_FLASH_ATTR int _close_filesystem_r(struct _reent *r, int fd)
{
    return SPIFFS_close(&fs, (spiffs_file)fd);
}

ICACHE_FLASH_ATTR int _unlink_r(struct _reent *r, const char *path)
{
    return SPIFFS_remove(&fs, path);
}

ICACHE_FLASH_ATTR int _fstat_r(struct _reent *r, int fd, void *buf)
{
    spiffs_stat s;
...
ICACHE_FLASH_ATTR int _stat_r(struct _reent *r, const char *pathname, void *buf)
{
    spiffs_stat s;
    struct stat *sb = (struct stat*)buf;
...
ICACHE_FLASH_ATTR off_t _lseek_r(struct _reent *r, int fd, off_t offset, int whence)
{
